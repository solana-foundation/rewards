...
use crate::{
    ...
    utils::{
        ...
        verify_readonly_or_writable_alias, verify_signer, verify_system_program, verify_token_account_owner,
...
impl<'a> TryFrom<&'a mut [AccountView]> for RevokeMerkleClaimAccounts<'a> {
    ...
    fn try_from(accounts: &'a mut [AccountView]) -> Result<Self, Self::Error> {
        ...
        verify_readonly_or_writable_alias(claimant, &[payer])?;
        verify_distinct_accounts(claimant, claim_account)?;
        verify_distinct_accounts(claimant, revocation_marker)?;
