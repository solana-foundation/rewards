...
pub struct CreateContinuousPoolData {
    ...
    pub revocable: u8,
    pub clawback_ts: i64,
    /// Non-zero to enable Token-2022 ConfidentialTransfer for reward payouts.
...
impl<'a> TryFrom<&'a [u8]> for CreateContinuousPoolData {
    ...
    fn try_from(data: &'a [u8]) -> Result<Self, Self::Error> {
        ...
        let confidential_rewards = data[11];

        Ok(Self { bump, balance_source, revocable, clawback_ts, confidential_rewards })
    }
}

impl<'a> InstructionData<'a> for CreateContinuousPoolData {
    const LEN: usize = 12; // bump(1) + balance_source(1) + revocable(1) + clawback_ts(8) + confidential_rewards(1)
