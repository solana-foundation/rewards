...
impl EventDiscriminator for PointsIssuedEvent {
    const DISCRIMINATOR: [u8; 8] = event_discriminator(b"PointsIssuedEvent");
}
...
mod tests {
    ...
    fn test_points_issued_event() {
        ...
        let event = PointsIssuedEvent::new(config, authority, seed, 1, 0, user, 500, 500);
    ...
    fn test_points_issued_event_to_bytes() {
        ...
        let seed = Address::new_from_array([4u8; 32]);
        let user = Address::new_from_array([2u8; 32]);
        let event = PointsIssuedEvent::new(config, authority, seed, 1, 1, user, 100, 100);
