...
impl PointsUsedEvent {
    pub const DATA_LEN: usize = 32 + 32 + 32 + 8 + 1 + 1 + 8 + 8 + 32 + 8 + 8; // 170
...
mod tests {
    ...
    fn test_points_used_event() {
        ...
        let event = PointsUsedEvent::new(config, authority, seed, 1_000_000, 1, 0, 500, 200, user, 200, 300);
    ...
    fn test_points_used_event_to_bytes() {
        ...
        let seed = Address::new_from_array([4u8; 32]);
        let user = Address::new_from_array([2u8; 32]);
        let event = PointsUsedEvent::new(config, authority, seed, 0, 1, 1, 100, 100, user, 100, 400);
